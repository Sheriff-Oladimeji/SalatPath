...
export const savePrayerLog = async (
  date: string,
  prayerLog: PrayerLog
): Promise<boolean> => {
  try {
...
export const areAllPrayersCompleted = async (
  date: string
): Promise<boolean> => {
  ...
  return (
    ...
    prayerLog.maghrib &&
    prayerLog.isha
    // Note: Tahajjud is optional and not required for streak
  );
};
...
export const getMonthPrayerLogs = async (
  year: number,
  month: number
  ...
  try {
    ...
    const prayerLogKeys = keys.filter((key) =>
      key.startsWith(PRAYER_LOG_KEY_PREFIX)
    );
    ...
    const monthKeys = prayerLogKeys.filter((key) =>
